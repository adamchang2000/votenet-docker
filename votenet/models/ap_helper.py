...
def parse_predictions(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(num_proposal):
            ...
            corners_3d_upright_camera = get_3d_box(box_size, pred_theta[i,j], pred_rot_vec[i,j], pred_center_upright_camera[i,j,:])
    ...
    for i in range(bsize):
        ...
        #else: 
        #changed to 0, only predict class 0
        #also don't allow per class proposal since only one class anyways
        batch_pred_map_cls.append([(0, pred_corners_3d_upright_camera[i,j], obj_prob[i,j]) \
            for j in range(pred_center.shape[1]) if pred_mask[i,j]==1 and obj_prob[i,j]>config_dict['conf_thresh']])
...
def parse_groundtruths(end_points, config_dict):
    ...
    rotation_vector_label = end_points['rotation_vector_label'].detach().cpu().numpy()
    theta_label = end_points['theta_label'].detach().cpu().numpy()
    ...
    for i in range(bsize):
        for j in range(K2):
            ...
            box_size = config_dict['dataset_config'].meanSize(0)
            corners_3d_upright_camera = get_3d_box(box_size, theta_label[i,j], rotation_vector_label[i,j], gt_center_upright_camera[i,j,:])
            gt_corners_3d_upright_camera[i,j] = corners_3d_upright_camera

    batch_gt_map_cls = []
    for i in range(bsize):
        batch_gt_map_cls.append([(0, gt_corners_3d_upright_camera[i,j]) for j in range(gt_corners_3d_upright_camera.shape[1]) if box_label_mask[i,j]==1])
